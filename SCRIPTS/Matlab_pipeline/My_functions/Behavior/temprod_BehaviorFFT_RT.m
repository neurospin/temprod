function temprod_BehaviorFFT_RT(subject,RunNum,fsample,TD)
...
subplot(4,3,1); plot(D); Title('successive estimation errors values');

% get histogram profile
T = max(D) - min(D);
subplot(4,3,4); hist(D,length(D)); title('estimation errors histogram');
[n1,xout1] = hist(D,T);

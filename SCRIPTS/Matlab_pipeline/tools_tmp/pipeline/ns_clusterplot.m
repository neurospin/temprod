function ns_clusterplot(cfg,stat,data1,data2,channel)
% function ns_clusterplot(cfg,stat,lat,data1,data2,channel) plots a series of topoplots with found clusters highlighted.
% stat is computed from FT_TIMELOCKSTATISTICS
% ns_clusterplot is adapted to NS from the fieldtrip function FT_CLUSTERPLOT
% (help below).
% 
% Marco Buiatti, Cognitive Neuroimaging Unit, Neurospin (2011).
      ...
      for iPl = 1:Nfig
          ...
          if is2D
                 ...
                 if strcmp(plotstyle,'dense')
                     for PlN=1:length(ind_timewin)-1
                      %PlN = (iPl-1)*15 + iT; %plotnumber
                      cfgtopo.xlim = [stat.time(ind_timewin(PlN)) stat.time(ind_timewin(PlN+1))];
                      % cfgtopo.highlightchannel   = list{PlN};
                      cfgtopo.comment = strcat('time: ',num2str(cfgtopo.xlim), ' s');
                      cfgtopo.commentpos = 'title';
                      subplot(length(ind_timewin)-1,3,3*(PlN-1)+1);
                      cfgtopo.highlightchannel   = [];
                      ft_topoplotER(cfgtopo, data1);
                      subplot(length(ind_timewin)-1,3,3*(PlN-1)+2);
                      cfgtopo.highlightchannel   = [];
                      ft_topoplotER(cfgtopo, data2);
                      cfgtopo.highlightchannel   = list{PlN};
                      subplot(length(ind_timewin)-1,3,3*(PlN-1)+3);
                      ft_topoplotER(cfgtopo, datadiff);
                     end
                 else
                     for PlN=1:length(ind_timewin)

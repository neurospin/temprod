function ns_clusterplot_timesmooth(cfg,stat,data1,data2,channel,tshw)
% function ns_clusterplot_timesmooth(cfg,stat,lat,data1,data2,channel,tshw) plots a series of topoplots with found clusters highlighted.
% stat is computed from FT_TIMELOCKSTATISTICS
% ns_clusterplot is adapted to NS from the fieldtrip function FT_CLUSTERPLOT
% (help below).
% INPUT:
% cfg, stat = see FT_CLUSTERPLOT help below
% data1(data2) = averaged data from ft_timelockanalysis or ft_timelockgrandaverage
...
%
% Example: ns_clusterplot_timesmooth(cfg,stat,av1,av2,'mag',0.02);
%
% Marco Buiatti, INSERM Cognitive Neuroimaging Unit, Neurospin (2011).

% select plot style
plotstyle='dense'; % every topography is the average of the ERF over a
% time interval. Do not use it, there's a bug, more soon...
%plotstyle='punctual'; % every topography is the ERF at one time point
% select channel set
load('/neurospin/meg/meg_tmp/tools_tmp/pipeline/SensorClassification.mat');
switch channel
    case 'mag'
        data1 = ft_selectdata(data1,'channel',Mag);
        data2 = ft_selectdata(data2,'channel',Mag);
    case 'grad1'
        data1 = ft_selectdata(data1,'channel',Grad_1);
        data2 = ft_selectdata(data2,'channel',Grad_1);
    case 'grad2'
        data1 = ft_selectdata(data1,'channel',Grad_2);
        data2 = ft_selectdata(data2,'channel',Grad_2);
    otherwise
        disp('Error: Incorrect channel selection!');
end;
data1.label=Mag;
data2.label=Mag;
tshwp=round(tshw/(data1.time(2)-data1.time(1)));
...
% options added by Marco for NS
cfgtopo.zlim = cfg.zlim;
cfgtopo.zparam='avg'; % overcomes previous field to plot data instead of stat
plotall=1; % plotall=0: plot topography of difference between conditions only with cluster sensors highlighted (figures: 3x5 subplots)
           % plotall=1: plot topography of the two conditions and
           % topography of the difference with cluster sensors highlighted
           % (figures: 3 columns (cond 1, cond 2 and difference), number of
           % rows = number of latency times)
% building the data difference
datadiff=data1;
...
if haspos == 0 && hasneg == 0
  fprintf('%s\n','no significant clusters in data; simply plotting the data')
  figure;
  if is2D
      % for iT = 1:Npl-(15*(Nfig-1))
      for PlN=1:length(ind_timewin)
          %PlN = (iPl-1)*15 + iT; %plotnumber
          cfgtopo.xlim = [stat.time(ind_timewin(PlN)) stat.time(ind_timewin(PlN))];
          % cfgtopo.highlightchannel   = list{PlN};
          cfgtopo.comment = strcat('time: ',num2str(stat.time(ind_timewin(PlN))), ' s');
          cfgtopo.commentpos = 'title';
          subplot(length(ind_timewin),3,3*(PlN-1)+1);
          % cfgtopo.highlightchannel   = [];
          ft_topoplotER(cfgtopo, data1);
          subplot(length(ind_timewin),3,3*(PlN-1)+2);
          % cfgtopo.highlightchannel   = [];
          ft_topoplotER(cfgtopo, data2);
          % cfgtopo.highlightchannel   = list{PlN};
          subplot(length(ind_timewin),3,3*(PlN-1)+3);
          ft_topoplotER(cfgtopo, datadiff);
      end
  else
...
%   end    
  ...
  if Nsigall==0
      fprintf('%s\n','Clusters in data not significant enough; simply plotting the data')
      figure;
      if is2D
          % for iT = 1:Npl-(15*(Nfig-1))
          for PlN=1:length(ind_timewin)
...
%   end
   if is2D
    for iPl = 1:Npl
      for iPos = 1:length(sigpos)
        list{iPl}{iPos} = find(sigposCLM(:,ind_timewin(iPl),iPos) == 1);
      end
      for iNeg = 1:length(signeg)
        list{iPl}{length(sigpos)+iNeg} = find(signegCLM(:,ind_timewin(iPl),iNeg) == 1);
      end
    end
  else
   for iPl = 1:Npl
  ...
  if plotall==0
      ...
      for iPl = 1:Nfig
          ...
          if is2D
                 % for iT = 1:Npl-(15*(Nfig-1))
                 if strcmp(plotstyle,'dense')
                     for PlN=1:length(ind_timewin)
                      %PlN = (iPl-1)*15 + iT; %plotnumber
                      cfgtopo.xlim = [stat.time(ind_timewin(PlN)-tshwp) stat.time(ind_timewin(PlN)+tshwp)];
                      % cfgtopo.highlightchannel   = list{PlN};
                      cfgtopo.comment = strcat('time: ',num2str(cfgtopo.xlim), ' s');
                      cfgtopo.commentpos = 'title';
                      subplot(length(ind_timewin),3,3*(PlN-1)+1);
                      cfgtopo.highlightchannel   = [];
                      ft_topoplotER(cfgtopo, data1);

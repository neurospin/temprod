function stat = ns_btcraf(tl1,tl2,tl3,channel,lat)
...
load('/neurospin/meg/meg_tmp/tools_tmp/pipeline/neighbours.mat');
...
cfg.statistic = 'indepsamplesF'; % use the independent samples F-statistic as a measure to evaluate 
...
design = zeros(1,size(tl1.trial,1) + size(tl2.trial,1) + size(tl3.trial,1));
design(1,1:size(tl1.trial,1)) = 1;
design(1,(size(tl1.trial,1)+1):(size(tl1.trial,1) + size(tl2.trial,1)))= 2;
design(1,(size(tl1.trial,1)+ size(tl2.trial,1)+1):(size(tl1.trial,1) + size(tl2.trial,1)+ size(tl3.trial,1)))= 3;
...
%compute the stat!
stat = ft_timelockstatistics(cfg, tl1, tl2, tl3);

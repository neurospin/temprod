function nuts = nut_fcdc2nuts(data,grid)
...
nuts.meg.sensorCoord = data.grad.pnt*1000; %10? in centimeters in fieldtrip
nuts.meg.Gcoef = 0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for j = 5:5:510
        ...
        NeuromagSensorOrient(tmp,:) = data.grad.ori(j,:);
%         NeuromagSensorCoord(tmp+1,:) = data.grad.pnt(j,:);
%         NeuromagSensorOrient(tmp+1,:) = data.grad.ori(j,:);
%         NeuromagSensorCoord(tmp+2,:) = data.grad.pnt(j,:);
%         NeuromagSensorOrient(tmp+2,:) = data.grad.ori(j,:);
%         tmp = tmp + 3;
        tmp = tmp + 1;
    end
    
    nuts.meg.sensorCoord = NeuromagSensorCoord;
    a = nuts.meg.sensorCoord(:,1);
    b = nuts.meg.sensorCoord(:,2);
    nuts.meg.sensorCoord(:,1) = b;
    nuts.meg.sensorCoord(:,2) = a;
%     
    nuts.meg.sensorOrient = NeuromagSensorOrient; 
    a = nuts.meg.sensorOrient(:,1);
    b = nuts.meg.sensorOrient(:,2);
    nuts.meg.sensorOrient(:,1) = b;
    nuts.meg.sensorOrient(:,2) = a;
    
    leftchans = find(NeuromagSensorCoord(:,1) >= 0);
    rightchans = find(NeuromagSensorCoord(:,1) < 0);
